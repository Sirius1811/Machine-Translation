...
def character_error_rate(predicted, expected):
    total_errors = 0
    total_chars = 0
    for pred, exp in zip(predicted, expected):
        total_errors += levenshtein_distance(pred, exp)
        total_chars += len(exp)
...
def word_error_rate(predicted, expected):
    ...
    total_words = 0
    for pred, exp in zip(predicted, expected):
        total_errors += levenshtein_distance(pred.split(), exp.split())
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    ...
    if writer:
        ...
        cer = character_error_rate(predicted, expected)
